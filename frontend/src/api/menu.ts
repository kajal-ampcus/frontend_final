 ...
 *   GET    /canteens/{canteen_id}/menu/categories/
 *   POST   /canteens/{canteen_id}/menu/categories/
 *   PATCH  /canteens/{canteen_id}/menu/categories/{id}/
 *   DELETE /canteens/{canteen_id}/menu/categories/{id}/
 *
 *   GET    /canteens/{canteen_id}/menu/items/
 *   POST   /canteens/{canteen_id}/menu/items/
 *   GET    /canteens/{canteen_id}/menu/items/{id}/
 *   PATCH  /canteens/{canteen_id}/menu/items/{id}/
 *   DELETE /canteens/{canteen_id}/menu/items/{id}/
 *   PATCH  /canteens/{canteen_id}/menu/items/{id}/availability/
 */
...
export interface ApiMenuItem {
  ...
  slot_configs: ApiMenuItemSlotConfig[];
  tags: string[];
  created_at: string;
  updated_at: string;
}

export interface ApiMenuItemSlotConfig {
  id: string;
  slot_id: string;
  slot_name: string;
...
export interface ReplaceMenuItemSlotConfigPayload {
  slots: Array<{
    slot_id: string;
    slot_name?: string;
    slot_start_time?: string | null;
...
export interface MenuItemSlotConfigListResponse {
  results: ApiMenuItemSlotConfig[];
}
...
export const canteenApi = {
  list: () => api.get<CanteenListResponse>("/canteens/").then((r) => r.data),
};

const categoryUrl = (canteenId: string, suffix = "") =>
  `/canteens/${canteenId}/menu/categories/${suffix}`;
...
const itemUrl = (canteenId: string, suffix = "") =>
  `/canteens/${canteenId}/menu/items/${suffix}`;
...
export const menuItemApi = {
  ...
  create: (canteenId: string, payload: CreateMenuItemPayload) =>
    api
      .post<ApiMenuItem>(itemUrl(canteenId), toMenuItemFormData(payload))
      .then((r) => r.data),

  /** PATCH — partial update */
  update: (canteenId: string, itemId: string, payload: UpdateMenuItemPayload) =>
    api
      .patch<ApiMenuItem>(itemUrl(canteenId, `${itemId}/`), toMenuItemFormData(payload))
      .then((r) => r.data),
  ...
  bulkImport: (canteenId: string, file: File) => {
    ...
    return api
      .post<BulkImportResponse>(itemUrl(canteenId, "bulk-import/"), formData)
      .then((r) => r.data);
  },
...
const slotConfigUrl = (canteenId: string, itemId: string) =>
  `/canteens/${canteenId}/menu/items/${itemId}/slots/`;

export const menuItemSlotApi = {
  list: (canteenId: string, itemId: string) =>
    api.get<MenuItemSlotConfigListResponse>(slotConfigUrl(canteenId, itemId)).then((r) => r.data),

  replace: (canteenId: string, itemId: string, payload: ReplaceMenuItemSlotConfigPayload) =>
    api
      .put<MenuItemSlotConfigListResponse>(slotConfigUrl(canteenId, itemId), payload)
      .then((r) => r.data),
};
