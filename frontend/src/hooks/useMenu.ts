...
import {
  ...
  type ApiCategory,
  type ApiMenuItem,
  type ApiMenuItemSlotConfig,
  type BulkImportResponse,
  type CreateCategoryPayload,
  type CreateMenuItemPayload,
  type MenuItemListParams,
  type ReplaceMenuItemSlotConfigPayload,
...
function readStoredCanteenId(): string {
  ...
  try {
    const selected = sessionStorage.getItem("canteen_selected_id");
    if (selected) return selected;

    const raw = sessionStorage.getItem("canteen_auth_claims");
    if (raw) {
  ...
  useEffect(() => {
    ...
    canteenApi
      ...
      .then((data) => {
        ...
        sessionStorage.setItem("canteen_selected_id", first.id);
        setCanteenId(first.id);
      })
      .catch(() => {
...
export function useCategories(): UseCategoriesReturn {
  ...
  const createCategory = useCallback(
    async (name: string): Promise<ApiCategory | null> => {
      if (!canteenId) {
        toast.error("Canteen is still loading. Please wait a moment and try again.");
        return null;
      }
      ...
      // Optimistic insert — gives instant feedback in the dropdown
      const tempId = `temp-${Date.now()}`;
      const optimistic: ApiCategory = { id: tempId, name: trimmed, is_active: true };
      setCategories((prev) => [...prev, optimistic]);

      try {
        const created = await categoryApi.create(canteenId, { name: trimmed });
        // Replace temp with real server row
...
interface UseMenuItemsReturn {
  ...
  getItemSlotConfigs: (itemId: string) => Promise<ApiMenuItemSlotConfig[]>;
  saveItemSlotConfigs: (
    itemId: string,
    payload: ReplaceMenuItemSlotConfigPayload,
  ) => Promise<ApiMenuItemSlotConfig[] | null>;
  refetch: () => void;
}

export function useMenuItems(params?: MenuItemListParams, canteenIdOverride?: string): UseMenuItemsReturn {
  const storedCanteenId = useCanteenId();
  const canteenId = canteenIdOverride || storedCanteenId;
  ...
  const getItemSlotConfigs = useCallback(
    async (itemId: string): Promise<ApiMenuItemSlotConfig[]> => {
      if (!canteenId) return [];
      const response = await menuItemSlotApi.list(canteenId, itemId);
      return response.results;
    },
    [canteenId],
  );

  const saveItemSlotConfigs = useCallback(
    async (
      itemId: string,
      payload: ReplaceMenuItemSlotConfigPayload,
    ): Promise<ApiMenuItemSlotConfig[] | null> => {
      if (!canteenId) return null;
      try {
        const response = await menuItemSlotApi.replace(canteenId, itemId, payload);
        setItems((current) =>
          current.map((item) =>
            item.id === itemId ? { ...item, slot_configs: response.results } : item,
          ),
        );
        return response.results;
      } catch (err: unknown) {
        const msg = extractApiError(err) ?? "Failed to save slot rules.";
        toast.error(msg);
        return null;
      }
    },
...
function mergeItem(item: ApiMenuItem, patch: UpdateMenuItemPayload): ApiMenuItem {
  return {
    ...
    ...(patch.max_qty_per_day !== undefined && { max_qty_per_day: patch.max_qty_per_day }),
