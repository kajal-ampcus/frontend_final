...
import { useAvailableSlots, useCmsPlaceOrder, useMyWallet } from "@/hooks/useCanteen";
import { formatINR, removeFromCart, updateCartQuantity, useCart } from "@/lib/store/index";
...
function CartPage() {
  ...
  const handleCheckoutSlot = async (slotId: string, slotName: string) => {
    ...
    const slot = mealSlots.find((candidate) => candidate.id === slotId);
    if (!slot?.canteen) {
      toast.error("This slot is not linked to an active canteen yet.");
      return;
    }
