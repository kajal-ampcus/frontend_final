...
const ACTIVE_ORDER_STATUSES: CmsOrder["status"][] = ["PENDING", "ACCEPTED", "PREPARING", "PREPARED"];

function Dashboard() {
  const navigate = useNavigate();
  const cart = useCart();
  const { data: mealSlots = [] } = useAvailableSlots();
  const { data: orders = [] } = useCmsOrderHistory();
  ...
  const todaysAnnouncements: Array<{
    id: string;
    title: string;
    message: string;
    fromTime?: string;
  ...
  const availableSlots = useMemo(
    () => mealSlots.filter((slot) => getSlotPhase(slot, currentTime) !== "expired"),
    [currentTime, mealSlots],
  );

  const selectedSlot = useMemo(
    () => mealSlots.find((slot) => slot.id === selectedSlotId) ?? null,
  ...
  const activeOrdersForSlot = useMemo(
    () =>
      userOrders.filter(
        (order) => order.slot === selectedSlot?.id && ACTIVE_ORDER_STATUSES.includes(order.status),
      ),
  ...
  const stats = {
    ...
    completedOrders: userOrders.filter((order) => order.status === "COLLECTED").length,
  ...
  useEffect(() => {
    const currentActive = mealSlots.find((slot) => getSlotPhase(slot, currentTime) === "active");
    if (currentActive) {
  ...
  }, [availableSlots, currentTime, mealSlots]);
  ...
  const topOrderedItems = useMemo(() => {
        ...
        order.order_items.forEach((item) => {
          ...
          if (!acc[item.menu_item]) {
            acc[item.menu_item] = {
              ...
              name: item.item_name_snapshot || "Meal",
              quantity: 0,
              orders: 0,
              total: 0,
              slotId: order.slot || null,
            };
          }

          acc[item.menu_item].quantity += item.quantity ?? 0;
          acc[item.menu_item].orders += 1;
          acc[item.menu_item].total += Number(item.line_total || 0);
          acc[item.menu_item].slotId = order.slot || acc[item.menu_item].slotId;
        });
    ...
    return Object.values(grouped)
      .map((entry) => ({
        ...entry,
        slot: entry.slotId ? (slotById[entry.slotId] ?? null) : null,
      }))
          ...
          <div className="rounded-[28px] border border-[#eadfce] bg-[linear-gradient(180deg,#fffdf9_0%,#fff8ef_100%)] p-6 shadow-[0_20px_45px_rgba(44,25,7,0.08)] dark:border-[#4a2f1e] dark:bg-[linear-gradient(180deg,#241710_0%,#17100b_100%)] dark:shadow-[0_24px_60px_rgba(0,0,0,0.32)]">
            ...
            <div className="mb-6 flex flex-wrap items-center gap-2">
              {availableSlots.map((slot) => {
                ...
                const isActive = getSlotPhase(slot, currentTime) === "active";
                    ...
                    >
                      {activeOrdersForSlot.map((order) => (
                        <option key={order.id} value={order.id}>
                          {order.order_code} - {order.order_items.map((item) => item.item_name_snapshot).join(", ")} - {formatINR(Number(order.total_amount))}
                        </option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div>
                    <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
                      <div className="flex items-center gap-3">
                        ...
                        <p className="text-sm font-medium text-[#7f6953] dark:text-[#cdb39b]">ID: {activeOrder.order_code}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-xl font-bold text-[#c7610e] dark:text-[#ffb467]">{formatINR(Number(activeOrder.total_amount))}</p>
                        <p className="text-xs text-[#907761] dark:text-[#b89a80]">
                          {activeOrder.order_items.reduce((sum, item) => sum + item.quantity, 0)} items
                        </p>
                      </div>
                    </div>
                      ...
                      <div className="rounded-[26px] border border-[#ecdcca] bg-[linear-gradient(180deg,#fff8ef_0%,#fffefb_100%)] p-5 dark:border-[#513424] dark:bg-[linear-gradient(180deg,#22160f_0%,#17100b_100%)]">
                        ...
                        <div className="mt-4 space-y-3">
                          {activeOrder.order_items.map((item) => (
                            ...
                            >
                              <div>
                                <p className="font-semibold text-[#2b1b10] dark:text-[#fff3e5]">{item.item_name_snapshot}</p>
            ...
            <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Based on your real order history from the backend.
                </p>
              </div>
              <button
                onClick={() => navigate({ to: "/menu" })}
            ...
            {topOrderedItems.length === 0 ? (
              ...
              <div className="grid gap-4 md:grid-cols-2">
                {topOrderedItems.map((entry) => {
                  const slotPhase = entry.slot ? getSlotPhase(entry.slot, currentTime) : "expired";
                  const isAvailable = Boolean(entry.slot && slotPhase !== "expired");
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          <h3 className="mt-3 text-lg font-bold text-[#26170d] dark:text-[#fff3e5]">{entry.name}</h3>
                          <p className="mt-1 text-sm text-[#7f6953] dark:text-[#cdb39b]">
                            One of your top repeated meals from previous real orders.
                          </p>
                        </div>
                        <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white text-primary shadow-sm dark:bg-[#2a1d16]">
                          {entry.name.toLowerCase().includes("veg") ? (
                      ...
                      <div className="mt-5 flex items-center justify-between gap-4">
                        <div>
                          <p className="text-xl font-bold text-[#c7610e] dark:text-[#ffb467]">
                            {formatINR(entry.total / Math.max(entry.quantity, 1))}
                          </p>
                          <p className="mt-1 text-xs text-[#907761] dark:text-[#b89a80]">Ordered {entry.quantity} time(s)</p>
                        </div>
                        <button
                          onClick={() => entry.slot && navigate({ to: "/menu", search: { slot: entry.slot.id } })}
                        ...
                        >
                          {isAvailable ? "Order In Slot" : "Unavailable"}
                        </button>
                      </div>
                    </div>
                  );
            ...
            <div className="rounded-[28px] border border-border bg-card p-6 shadow-sm dark:border-[#37231a] dark:bg-[#16100d]">
              ...
              <div className="grid gap-4">
                {mealSlots.map((slot) => {
                  ...
                  const phase = getSlotPhase(slot, currentTime);
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="mt-1 text-sm text-muted-foreground">
                              {formatApiTime(slot.start_time)} - {formatApiTime(slot.end_time)}
                            </p>
                          </div>
                        </div>
                      ...
                      <button
                        onClick={() => !isExpired && navigate({ to: "/menu", search: { slot: slot.id } })}
                        disabled={isExpired}
...
function getStatusSteps(status: CmsOrder["status"]) {
...
function getDisplayStatusLabel(status: CmsOrder["status"]) {
  switch (status) {
    case "PENDING":
    case "ACCEPTED":
      return "Placed";
...
function getSlotPhase(slot: TimeSlot, currentTime: Date) {
  ...
  if (!slot.is_active || nowMinutes > endMinutes) return "expired";
