...
class MenuItemSlotConfigSerializer(serializers.ModelSerializer):
    class Meta:
        model = MenuItemSlotConfig
        fields = [
            'id',
            'slot_id',
            'slot_name',
...
class MenuItemSlotConfigWriteSerializer(serializers.Serializer):
    slot_id = serializers.CharField(max_length=100)
    slot_name = serializers.CharField(max_length=100, required=False, allow_blank=True)
    slot_start_time = serializers.TimeField(required=False, allow_null=True)
    slot_end_time = serializers.TimeField(required=False, allow_null=True)
    quantity_per_slot = serializers.IntegerField(min_value=1)
    max_qty_per_order = serializers.IntegerField(min_value=1)
    max_qty_per_person = serializers.IntegerField(min_value=1)
    max_qty_per_day = serializers.IntegerField(min_value=1, required=False, allow_null=True)
...
class MenuItemSlotConfigReplaceSerializer(serializers.Serializer):
    slots = MenuItemSlotConfigWriteSerializer(many=True)
    ...
    def validate_slots(self, value):
        ...
        for item in value:
            ...
            if slot_id in seen:
                raise serializers.ValidationError(f"Duplicate slot config for slot_id '{slot_id}'.")
            seen.add(slot_id)
        return value
...
class MenuItemSerializer(serializers.ModelSerializer):
    ...
    slot_configs = MenuItemSlotConfigSerializer(many=True, read_only=True)
    ...
    class Meta:
        ...
        fields = [
            ...
            'max_qty_per_order',
            'max_qty_per_day',   # "Quantity to be Served"
            'slot_configs',
            'tags',
            'created_at',
            'updated_at',
...
class MenuItemCreateSerializer(serializers.ModelSerializer):
    ...
    UI → field mapping:
      ...
      "Quantity to be Served" → max_qty_per_day   (required by UI, nullable in DB)
    ...
    class Meta:
        ...
        extra_kwargs = {
            ...
            'max_qty_per_day':   {'required': False, 'allow_null': True},
            'unit':              {'required': False},
            'is_available':      {'required': False},
            'max_qty_per_order': {'required': False},
            'tags':              {'required': False},
    ...
    def validate_max_qty_per_day(self, value):
        if value is not None and value <= 0:
            raise serializers.ValidationError("Quantity to be served must be greater than 0.")
        return value

    def validate_max_qty_per_order(self, value):
        if value <= 0:
            raise serializers.ValidationError("Max quantity per order must be greater than 0.")
        return value

    def validate_item_type(self, value):
        allowed = [MenuOrderItem.ITEM_TYPE_BREAKFAST, MenuOrderItem.ITEM_TYPE_MEAL]
    ...
    def validate_name(self, value):
        ...
        canteen_id = self.context.get('canteen_id')
        qs = MenuOrderItem.all_objects.filter(
    ...
    def create(self, validated_data):
        ...
        return MenuOrderItem.objects.create(**validated_data)
