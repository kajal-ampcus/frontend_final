...
from apps.cms.models import MenuCategory, MenuOrderItem, MenuItemSlotConfig
from apps.cms.serializers.menu import (
    ...
    MenuItemSlotConfigSerializer,
    MenuItemSlotConfigReplaceSerializer,
)
...
def canteen_list_view(request):
    """
    GET /cms/canteens/
    Returns active canteens available to the logged-in user.

    POST /cms/canteens/
    Creates a new canteen (CMS Admin only).
    """
    ...
    # POST: Create a new canteen (Admin only)
    if request.method == 'POST':
        if role_type not in RoleChoices.CMS_ADMIN_ROLES:
            return Response({'detail': 'Admin access required.'}, status=status.HTTP_403_FORBIDDEN)

        name = request.data.get('name', '').strip()
        if not name:
            return Response({'detail': 'Canteen name is required.'}, status=status.HTTP_400_BAD_REQUEST)

        # Use company_id from JWT or request body
        target_company_id = request.data.get('company_id') or company_id
...
def menu_item_list_create_view(request, canteen_id):
    ...
    if request.method == 'GET':
        qs = (
            MenuOrderItem.objects                       # ActiveManager → deleted_at IS NULL
        ...
        if item_type in (MenuOrderItem.ITEM_TYPE_BREAKFAST, MenuOrderItem.ITEM_TYPE_MEAL):
...
def menu_item_bulk_import_view(request, canteen_id):
    ...
    CSV columns:
      name, base_price, max_qty_per_day, category, item_type,
      description, display_tag, unit, max_qty_per_order, is_available
    ...
    for index, row in enumerate(rows, start=2):
        ...
        if MenuOrderItem.all_objects.filter(
            canteen=canteen,
        ...
        payload = {
            ...
            'max_qty_per_day': (row.get('max_qty_per_day') or '').strip(),
            'category_id': str(category.id),
            'item_type': (row.get('item_type') or MenuOrderItem.ITEM_TYPE_MEAL).strip().upper(),
            'display_tag': (row.get('display_tag') or '').strip(),
            'unit': (row.get('unit') or 'plate').strip(),
            'max_qty_per_order': (row.get('max_qty_per_order') or 3),
...
@api_view(['GET', 'PUT'])
@permission_classes([IsAuthenticated, IsEmployeeOrAdmin])
def menu_item_slot_config_view(request, canteen_id, item_id):
    """
    GET /cms/canteens/{canteen_id}/menu/items/{item_id}/slots/
    PUT /cms/canteens/{canteen_id}/menu/items/{item_id}/slots/
    """
    canteen = _get_canteen_or_404(canteen_id, request)
    if canteen is None:
        return Response({'detail': 'Canteen not found.'}, status=status.HTTP_404_NOT_FOUND)
    ...
    if request.method == 'GET':
        slot_configs = item.slot_configs.filter(is_active=True).order_by('slot_name', 'slot_id')
        return Response(
            {'results': MenuItemSlotConfigSerializer(slot_configs, many=True).data},
            status=status.HTTP_200_OK,
        )
    ...
    serializer = MenuItemSlotConfigReplaceSerializer(data=request.data)
    if not serializer.is_valid():
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    MenuItemSlotConfig.objects.filter(menu_item=item, canteen=canteen).delete()
    ...
    slot_rows = [
        ...
        for slot_data in serializer.validated_data['slots']
    ]
    if slot_rows:
        MenuItemSlotConfig.objects.bulk_create(slot_rows)

    refreshed = MenuItemSlotConfig.objects.filter(
        menu_item=item,
        canteen=canteen,
        is_active=True,
    ).order_by('slot_name', 'slot_id')
    return Response(
        {'results': MenuItemSlotConfigSerializer(refreshed, many=True).data},
        status=status.HTTP_200_OK,
    )
