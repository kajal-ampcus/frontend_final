...
Admin dashboard and reporting endpoints for CanteenX CMS.
...
def _serialize_order(order, include_items=True):
...
def admin_dashboard_view(request):
    ...
    - todayOrders: count of today's orders (excluding cancelled)
    - todayRevenue: sum of today's order totals
    - slotCounts: orders per slot
    - statusCounts: orders by status (live orders only)
    - activeUsers: unique employees who ordered today
    - avgProcessingTime: average time from placed to delivered (in minutes)
    """
    ...
    # Base queryset for today's orders
    ...
    # Active users (unique employees who ordered today)
    active_users = today_orders.values('employee_id').distinct().count()
    ...
    for slot in slots:
        count = today_orders.filter(slot=slot).count()
        slot_counts.append({
            'slotId': str(slot.id),
            'slot': slot.name,
            'orders': count
    ...
    # Status distribution (live orders only - not delivered/cancelled)
    ...
    status_counts = {
        'pending': live_orders.filter(status=OrderStatus.PLACED).count(),
        'preparing': live_orders.filter(status=OrderStatus.PREPARING).count(),
        'ready': live_orders.filter(status=OrderStatus.READY).count(),
        'delivered': today_orders.filter(status=OrderStatus.DELIVERED).count(),
    }

    # Average processing time (placed to delivered for today's delivered orders)
    ...
    if delivered_orders.exists():
        ...
        for order in delivered_orders:
            if order.collected_at and order.placed_at:
                ...
                count += 1
        if count > 0:
            avg_time_minutes = int(total_time.total_seconds() / 60 / count)
    ...
    return Response({
        ...
        'avgProcessingTime': avg_time_minutes,
    })
...
def admin_debug_slots_view(request):
    ...
    for slot in all_slots:
        ...
        if slot.label == 'CLOSED':
            issues.append("label='CLOSED': Slot is marked as closed")
            passes_filter = False
