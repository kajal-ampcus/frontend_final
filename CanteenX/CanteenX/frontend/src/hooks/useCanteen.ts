...
const mock = {
  ...
  MOCK_CMS_LOCATIONS: [] as any[],
  MOCK_CATEGORIES: [] as any[],
  MOCK_ITEMS: [] as any[],
  MOCK_BREAK_SLOTS: [] as any[],
  MOCK_SLOT_TYPES: [] as any[],
  MOCK_TIME_SLOTS: [] as any[],
  MOCK_CMS_ORDERS: [] as any[],
  MOCK_KITCHEN_BOARD: {
    accepted: [] as any[],
    preparing: [] as any[],
    prepared: [] as any[],
  ...
  MOCK_KITCHEN_DASHBOARD: {} as Record<string, unknown>,
  MOCK_WALLET: {
    id: '',
    employee: '',
    balance: 0,
  ...
  MOCK_TRANSACTIONS: [] as any[],
  MOCK_MY_ORDERS: [] as any[],
  MOCK_ORDER_HISTORY: [] as any[],
  MOCK_ESS_DASHBOARD: {
    active_order: null,
    today_slots: [] as any[],
    monthly_spend: '0.00',
  ...
  MOCK_SLOT_MENUS: {} as Record<string, any[]>,
  MOCK_ORDERING_RULES: [] as any[],
  MOCK_GUEST_MEALS_STATE: [] as any[],
  MOCK_EMPLOYEES: [] as any[],
  MOCK_EMPLOYEES_STATE: [] as any[],
  MOCK_ITEM_SLOT_AVAILABILITY: [] as any[],
  MOCK_ORDERS_STATE: [] as any[],
  MOCK_BILLING: [] as any[],
  setMockGuestMealsState(next: any[]) {
    mock.MOCK_GUEST_MEALS_STATE = next;
  },
  setMockEmployeesState(next: any[]) {
    mock.MOCK_EMPLOYEES_STATE = next;
  },
  setMockItemSlotAvailability(next: any[]) {
    mock.MOCK_ITEM_SLOT_AVAILABILITY = next;
  },
  setMockOrdersState(next: any[]) {
    mock.MOCK_ORDERS_STATE = next;
  },
};
...
export function useCanteenLocations() {
  return useQuery({
    ...
    queryFn: async () => mock.MOCK_LOCATIONS as CanteenLocation[],
  });
}
...
export function useGuestOrders(params?: { status?: string; search?: string; page?: number; page_size?: number }) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await api.get(`/guest-orders/?${query.toString()}`);
      return (data.results ?? data) as GuestOrder[];
    },
...
export function useMenuAvailable(params?: { slot?: string; category?: string; search?: string; day?: string }) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await api.get(`/menu/available/?${query.toString()}`);
      return (data.results ?? data) as MenuItem[];
    },
    keepPreviousData: true,
  });
}
...
export function useMenuSlots() {
  return useQuery({
    queryKey: ['menu', 'slots'],
    queryFn: async () => {
      const { data } = await api.get('/menu/slots/');
...
export function useCmsPlaceOrder() {
  ...
  return useMutation({
    mutationFn: async (payload: CmsPlaceOrderPayload): Promise<CmsOrder> => {
      ...
      mock.setMockOrdersState([newOrder, ...mock.MOCK_ORDERS_STATE]);
      return newOrder;
    },
    onSuccess: () => {
...
export function useCmsOrderHistory(params?: {
  ...
  return useQuery({
    ...
    queryFn: async () => mock.MOCK_ORDER_HISTORY as CmsOrder[],
  });
}
...
export function useCreateGuestMeal() {
  ...
  return useMutation({
    mutationFn: async (payload: GuestMealPayload): Promise<GuestMeal> => {
      ...
      mock.setMockGuestMealsState([newMeal, ...mock.MOCK_GUEST_MEALS_STATE]);
      return newMeal;
    },
...
export function useDeleteGuestMeal() {
  ...
  return useMutation({
    mutationFn: async (id: string): Promise<void> => {
      mock.setMockGuestMealsState(mock.MOCK_GUEST_MEALS_STATE.filter((m) => m.id !== id));
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['canteen', 'guest-meals'] }); },
  });
}
...
export function useAssignItemToSlots() {
  ...
  return useMutation({
    mutationFn: async ({ itemId, slotIds }: { itemId: string; slotIds: string[] }): Promise<void> => {
      mock.setMockItemSlotAvailability(mock.MOCK_ITEM_SLOT_AVAILABILITY.filter((a) => a.item_id !== itemId));
...
export function useCreateEmployee() {
  ...
  return useMutation({
    mutationFn: async (payload: Omit<Employee, 'id' | 'created_at'>): Promise<Employee> => {
      ...
      mock.setMockEmployeesState([...mock.MOCK_EMPLOYEES_STATE, emp]);
      return emp;
    },
