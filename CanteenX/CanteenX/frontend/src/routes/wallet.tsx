...
import { fetchEmployeeWallet, rechargeEmployeeWallet, type EmployeeWallet } from "@/api/wallet";
...
function WalletPage() {
  ...
  const handleAddFunds = async () => {
    ...
    try {
      const updated = await rechargeEmployeeWallet(amount);
      setWallet(updated);
      toast.success(`Added ${formatINR(amount)} to your wallet`);
      setAddAmount("");
      setShowAddFunds(false);
    } catch (error) {
      toast.error((error as Error).message || "Could not add funds.");
    }
  };
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary via-orange-500 to-amber-500 p-6 text-white shadow-xl shadow-primary/25 md:col-span-2">
            ...
            <div className="relative">
              ...
              <p className="mt-2 text-4xl font-bold">{formatINR(wallet?.balance ?? currentCustomer?.walletBalance ?? 0)}</p>
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell className="whitespace-nowrap text-right font-bold text-muted-foreground">
                      {formatINR(transaction.balance ?? 0)}
                    </TableCell>
                  </TableRow>
                );
              })}
