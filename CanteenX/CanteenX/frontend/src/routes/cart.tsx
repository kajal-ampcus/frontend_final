...
function CartPage() {
  ...
  useEffect(() => {
    let ignore = false;
    Promise.all([fetchEmployeeMenu(), fetchEmployeeWallet()])
  ...
  const handleCheckoutSlot = async (slotId: string, slotName: string) => {
    ...
    if ((wallet?.balance ?? currentCustomer?.walletBalance ?? 0) < slotTotal) {
            ...
            <div className="sticky top-24 h-fit rounded-2xl border bg-card p-6">
              <div className="space-y-4">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">{formatINR(wallet?.balance ?? currentCustomer?.walletBalance ?? 0)}</span>
                </div>
              ...
              <div className="mt-8 space-y-3">
                {slotGroups.map(([slotId, items]) => {
                  const slotName = items[0]?.slotName ?? "General";
                  const slotTotal = items.reduce((sum, item) => sum + item.price * item.quantity, 0);
                  const canAfford = (wallet?.balance ?? currentCustomer?.walletBalance ?? 0) >= slotTotal;

                  return (
                    <button
                      key={`checkout-${slotId}`}
                      onClick={() => handleCheckoutSlot(slotId, slotName)}
                      disabled={!canAfford || checkingOutSlotId === slotId}
                      className={`w-full rounded-xl py-3.5 font-bold text-white transition-all ${
                        canAfford && checkingOutSlotId !== slotId
                          ? "bg-primary shadow-lg shadow-primary/25 hover:bg-primary/90 hover:shadow-primary/40 active:scale-[0.98]"
                          : "cursor-not-allowed bg-muted text-muted-foreground"
                      }`}
                    >
                      {checkingOutSlotId === slotId ? "Placing order..." : `Checkout ${slotName} - ${formatINR(slotTotal)}`}
                    </button>
                  );
                })}
              </div>
