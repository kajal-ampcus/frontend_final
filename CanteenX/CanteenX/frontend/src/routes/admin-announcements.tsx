...
function AdminAnnouncements() {
  ...
  const availableDishes = useMemo(
    () => {
      const menuDishes = [...new Set(menu.filter((item) => item.available).map((item) => item.name))].sort(
        (left, right) => left.localeCompare(right),
      );
      return [...menuDishes, ...customDishes];
    },
    [menu, customDishes],
  );
  ...
  const handleAddNewDish = (dishName: string) => {
    const trimmedName = dishName.trim();
    if (trimmedName && !customDishes.includes(trimmedName) && !availableDishes.includes(trimmedName)) {
      setCustomDishes((prev) => [...prev, trimmedName]);
                          ...
                          <div className="max-h-48 overflow-y-auto">
                            ...
                            {filteredDishes.map((dish) => (
                              <button
                                ...
                                className={cn(
                                  "w-full px-3 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-[#2a1f15]",
                            ...
                            {(form.dishSearchInput || "").trim() && !filteredDishes.includes((form.dishSearchInput || "").trim()) && (
                              <button
                                type="button"
                                onClick={() => handleAddNewDish(form.dishSearchInput || "")}
