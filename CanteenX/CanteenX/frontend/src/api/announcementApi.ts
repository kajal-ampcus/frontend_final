...
async function parseResponse<T>(res: Response): Promise<T> {
  ...
  if (!res.ok) {
    const msg =
      (data as any)?.detail ??
      // DRF field errors — flatten to first message for toast display
      Object.values((data as any) ?? {}).flat()[0] ??
      res.statusText ??
      'Request failed';
    const err = new Error(String(msg)) as Error & { status: number; data: unknown };
...
export async function updateAnnouncement(
  ...
  const data = await http<BackendAnnouncement>('PUT', `/${id}/`, toBackendPayload(input));
  return toFrontend(data);
}
