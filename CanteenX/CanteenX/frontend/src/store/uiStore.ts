...
import { getTokenFromStorage, getClaimsFromStorage } from '@/lib/authStorage';
...
function mapRoleType(roleType: string): UserRole {
  ...
  if (roleType === 'COMPANY_ADMIN' || roleType === 'HR_MANAGER' || roleType === 'PAYROLL_MANAGER') return 'admin';
    ...
    (set) => ({
      ...
      logout: () => {
        if (typeof window !== "undefined") {
          sessionStorage.removeItem('canteen_auth_token');
          sessionStorage.removeItem('canteen_auth_claims');
          sessionStorage.removeItem('canteen_auth_session');
        }
        set({
          isAuthenticated: false,
          authToken: null,
