...
function AdminSlots() {
  ...
  const handleViewSlotItems = (slot: Slot) => {
  ...
  const handleToggleSlotItem = (slotId: string, itemId: string) => {
    ...
    const updatedDisabled = disabledItemIds.includes(itemId)
  ...
  const handleSaveSlot = (data: Partial<Slot>) => {
    if (editingSlot) {
      ...
      createEntity<Slot>("slot", {
        ...
        displayTime: data.displayTime ?? `${data.startTime ?? "09:00"} — ${data.endTime ?? "11:00"}`,
        type: data.type ?? "Meal",
        defaultCategory: data.defaultCategory ?? "Veg",
